    ...
    <main className="tw:min-h-screen tw:bg-[#e5e4e2] tw:px-4 tw:py-24">
      <div className="tw:mx-auto tw:max-w-5xl">
        ...
        <section className="tw:mt-6 tw:rounded-2xl tw:border tw:border-gray-200 tw:bg-white tw:p-4">
          ...
          <span className="tw:block tw:text-lg tw:font-medium">Active check-in staff</span>
          </div>
        ...
        {activeAccess ? <button style={{ borderRadius: 36, fontSize: 12 }} className="tw:px-4 tw:py-2 tw:border tw:border-red-300 tw:hover:bg-red-100 tw:mt-4 tw:text-sm tw:text-red-700" onClick={() => perform("revoke", api.patch(`/api/v1/events/${eventId}/checkin-access/${activeAccess.id}/revoke`, {}, authHeaders(token)))} disabled={!!pending}>Revoke current code ending in {activeAccess.plain_code_last_four}</button> : null}
      </div>
    </main>
  );
}
